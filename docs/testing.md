...
| `npm run test:e2e` | Build, preview the production output, and run functional/component Playwright tests. |
| `npm run test:a11y` | Build, preview the production output, and run axe accessibility checks. |
| `npm run test:visual` | Build, preview the production output, and run visual snapshots. |
...
Playwright owns the server used by browser tests. It builds the static site
with the suite's configured Payload environment and serves `dist/` with `astro
preview`. Tests do not use `astro dev`.

Do not start a separate dev or preview process on port 4321 before a browser
run. A build failure or unavailable CMS is a real test failure; do not work
around it by reusing an unrelated server.

`wcag-front` and `a11ying-front` share port 4321. Run their full quality gates
